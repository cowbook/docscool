...
## 1.3 本轮新增回归项（2026-05-18 用户权限同步策略调整）
- 用户组同步回归（add-only）：
	- 回归 `GET /api/settings/users` 前同步仅执行“数据库用户缺失则补入 docscool 组”。
	- 回归数据库无新增用户时，不触发群组 `set` 调用。
	- 回归群组已有但数据库缺失的用户不被移除（确认已取消清理逻辑）。
	- 回归不再自动补录当前登录用户。
- 用户删除回归：
	- 回归前端用户权限表“删除”按钮可用、确认弹窗文案正确。
	- 回归 `DELETE /api/settings/users/<id>` 删除数据库记录成功。
	- 回归删除后触发一次 docscool 组补齐同步，接口返回 `warnings` 结构稳定。
- 用户详情调试日志回归：
	- 回归 `_synology_get_user_info` 的 GET/POST 请求日志与响应摘要日志均输出。
	- 回归日志包含 `attempt / success / code / users_count` 关键字段。

## 1.4 下次优化任务（2026-05-18）
- 用户-用户组关系验证 API 选型：
	- 需要进一步测试 Synology 中“真正能正确验证用户名与用户组关系”的 API。
	- 重点排查 `SYNO.Core.Group.Member.get_users`、`SYNO.Core.Group` 以及相关成员接口在当前 DSM 版本的真实行为。
	- 目标是找到一个能稳定验证“用户是否属于 docscool 组”的方式，然后替换当前的回读/容错逻辑。
	- 下次优化时不要再使用“验证不可用就直接接受”的策略。
