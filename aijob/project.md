...
## 当前前端交互约定
- 全屏文件预览弹窗标题需显示存储目录面包路径（`/` 分隔），支持长路径省略与悬浮完整提示。
...
## 本轮 OCR 预览与编辑器升级（2026-05-26）
- OCR 预览路由统一为目录形式：`/docs/preview/<encoded_dir>/`，并自动映射到同目录 `full.md`。
- 左侧 PDF 预览链路改造：
  - `html-meta` 优先返回 `_origin.pdf`；缺失时回退到存储目录同名 PDF。
  - 受保护预览地址采用“前端带 token 预取 blob -> iframe 显示”，避免 `Missing token/404` 闪现。
  - 无可预览文件时，先显示“加载中...”，仅在最终无结果时显示空态提示。
- 右侧编辑器升级为 Toast UI（WYSIWYG），保留图片上传能力：
  - 编辑器容器使用 `v-show` 保持实例，修复“编辑 -> 查看 -> 编辑”空白问题。
  - 工具栏改为苹果扁平风格：独立 `AI识别/重新AI识别` 按钮 + 编辑按钮组（查看/编辑、保存）。
  - `AI识别` 识别中显示小转圈动画。
- 无 OCR `full.md` 时：
  - 禁用“进入编辑”；
  - 提供 `AI识别` 按钮调用 `POST /api/contracts/ai-parse`，成功后自动刷新并解锁编辑。
- 编辑态相对路径图片修复：
  - 进入编辑时将相对路径临时映射为 `/api/html/...` 资源地址以保证可见；
  - 保存时再还原为相对路径，避免污染源 markdown。
