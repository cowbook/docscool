...
- 合同关联字段落地：
	...
	- 合同字段新增 `contract_form`（合同形式），选项为：新签合同、补充合同、补充协议、变更合同。
- 前端可视化联动：
	- 合同列表页新增“原合同”列，显示“合同编号 - 合同名称”。
	- 文件档案页（FolderView）表格新增“原合同”列，支持按该字段搜索。
	- 合同编辑弹窗新增“当前关联”提示，便于快速确认原合同关系。
	- 文件档案页弹出的合同编辑器已接入合同候选列表，可直接选择原合同。
- 启动稳定性修复：
	- 处理旧库启动报错 `no such column: contracts.original_contract_id`，在后端启动补列逻辑中补充该字段自动迁移。
	- 修复后后端可正常启动。
...
- MIS 合同详情接口调用口径确认：
	...
	- 接口成功响应不保证返回完整合同主字段；实测存在仅返回 `{"payment":[{}]}` 的场景，调用方需按“部分数据可用”处理。
